...
package org.eclipse.tracecompass.incubator.internal.dependency.analysis.ui.views.controlflow;
...
import org.eclipse.tracecompass.incubator.dependency.analysis.core.matching.SpanDependency;
...
public class ControlFlowView extends AbstractStateSystemTimeGraphView {
    ...
    private @Nullable List<ILinkEvent> fLinkList = null;
    ...
    protected @Nullable ZoomThread createZoomThread(long startTime, long endTime, long resolution, boolean restart) {
        fLinkList = null;
        return super.createZoomThread(startTime, endTime, resolution, restart);
    }

    @Override
    protected @NonNull List<ILinkEvent> getLinkList(ITmfStateSystem ss,
            @NonNull List<List<ITmfStateInterval>> fullStates, @Nullable List<ITmfStateInterval> prevFullState, @NonNull IProgressMonitor monitor) {
        List<ILinkEvent> list = fLinkList;
        if (list != null) {
            return list;
        }
        list = new ArrayList<>();
        ...
        SpanDependencyAnalysis analysis = TmfTraceUtils.getAnalysisModuleOfClass(trace2, SpanDependencyAnalysis.class, SpanDependencyAnalysis.ID);
        if (analysis == null) {
            return list;
        }
        analysis.schedule();
        /* In the meantime, gather the entries */
        List<TimeGraphEntry> entryList = new ArrayList<>();
        for (ITmfTrace trace : TmfTraceManager.getTraceSet(trace2)) {
            ITmfStateSystem ssq = TmfStateSystemAnalysisModule.getStateSystem(trace, KernelAnalysisModule.ID);
            if (ssq != null) {
                List<@NonNull TimeGraphEntry> entryList2 = getEntryList(ssq);
                if (entryList2 != null) {
        ...
        if (entryList.isEmpty() || segmentStore == null) {
            return list;
        }
        for (ISegment segment : segmentStore) {
            ...
            ITimeGraphEntry prevEntry = findEntry(entryList, dep.getSource().getHost(), dep.getSource().getTid());
            ITimeGraphEntry nextEntry = findEntry(entryList, dep.getDestination().getHost(), dep.getDestination().getTid());
    ...
    private ControlFlowEntry findEntry(List<TimeGraphEntry> entryList, String hostId, int threadId) {
        for (TimeGraphEntry entry : entryList) {
            if (entry instanceof ControlFlowEntry) {
                ControlFlowEntry controlFlowEntry = (ControlFlowEntry) entry;
                if (controlFlowEntry.getThreadId() == threadId && controlFlowEntry.getTrace().getHostId().equals(hostId)) {
                    return controlFlowEntry;
                }
            }
            if (entry.hasChildren()) {
                ControlFlowEntry controlFlowEntry = findEntry(entry.getChildren(), hostId, threadId);
