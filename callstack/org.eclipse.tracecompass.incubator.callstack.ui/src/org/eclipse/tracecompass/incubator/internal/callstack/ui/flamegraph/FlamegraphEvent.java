...
import org.eclipse.tracecompass.incubator.internal.callstack.core.callgraph.instrumented.ICalledFunction;
    ...
    public Map<String, String> getTooltip(Format formatter) {
        ...
        for (Entry<String, IStatistics<ICalledFunction>> entry : fCallSite.getStatistics().entrySet()) {
            ...
            if (stats.getMax() != IHostModel.TIME_UNKNOWN) {
                ...
                builder.put("\t" + Messages.FlameGraph_Total + ' ' + statType, formatter.format(stats.getTotal())); //$NON-NLS-1$
                builder.put("\t" + Messages.FlameGraph_Average + ' ' + statType, formatter.format(stats.getMean())); // $NON-NLS-1$ //$NON-NLS-1$
                builder.put("\t" + Messages.FlameGraph_Max + ' ' + statType, formatter.format(stats.getMax())); // $NON-NLS-1$ //$NON-NLS-1$
                builder.put("\t" + Messages.FlameGraph_Min + ' ' + statType, formatter.format(stats.getMin())); // $NON-NLS-1$ //$NON-NLS-1$
                builder.put("\t" + Messages.FlameGraph_Deviation + ' ' + statType, formatter.format(stats.getStdDev())); //$NON-NLS-1$
