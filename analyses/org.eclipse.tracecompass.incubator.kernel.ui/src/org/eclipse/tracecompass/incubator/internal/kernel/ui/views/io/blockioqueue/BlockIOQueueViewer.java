...
package org.eclipse.tracecompass.incubator.internal.kernel.ui.views.io.blockioqueue;
...
public class BlockIOQueueViewer extends TmfCommonXLineChartViewer {
     ...
     *            parent view
     */
    public BlockIOQueueViewer(Composite parent) {
        super(parent, Messages.BlockIOQueueViewer_Title, Messages.BlockIOQueueViewer_XAxis, Messages.BlockIOQueueViewer_YAxis);
    ...
    protected void updateData(long start, long end, int nb, IProgressMonitor monitor) {
        try {
            ...
            while (!complete && currentEnd < end) {
                ...
                for (Integer diskQuark : ss.getSubAttributes(disksQuark, false)) {
                    String diskname = ss.getAttributeName(diskQuark);
                    int blockIOQueueQuark = ss.getQuarkRelative(diskQuark, Attributes.WAITING_QUEUE_LENGTH);
                    ...
                    /*
                     * TODO: It should only show active threads in the time
                     * range. If a tid does not have any memory value (only 1
                     * interval in the time range with value null or 0), then
                     * its series should not be displayed.
                     */
                    double yvalue = 0.0;
                    for (int i = 0; i < xvalues.length; i++) {
                        ...
                        // make sure that time is in the trace range after
                        // double to
                        // long conversion
                        ...
                        try {
                            yvalue = ss.querySingleState(time, blockIOQueueQuark).getStateValue().unboxLong();
                            fYValues[i] = yvalue;
